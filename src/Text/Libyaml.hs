...
foreign import ccall unsafe "get_start_line"
    c_get_start_line :: EventRaw -> IO CInt

foreign import ccall unsafe "get_end_line"
    c_get_end_line :: EventRaw -> IO CInt

foreign import ccall unsafe "get_start_column"
    c_get_start_column :: EventRaw -> IO CInt

foreign import ccall unsafe "get_end_column"
    c_get_end_column :: EventRaw -> IO CInt

getEvent :: EventRaw -> IO (Maybe (Position, Event))
getEvent er = do
    et <- c_get_event_type er
    pos <- Position <$> (fromIntegral `fmap` c_get_start_line er)
                    <*> (fromIntegral `fmap` c_get_end_line er)
                    <*> (fromIntegral `fmap` c_get_start_column er)
                    <*> (fromIntegral `fmap` c_get_end_column er)
    let ok e = return $ Just (pos, e)
    case toEnum $ fromEnum et of
        YamlNoEvent -> return Nothing
        YamlStreamStartEvent -> ok EventStreamStart
        YamlStreamEndEvent -> ok EventStreamEnd
        YamlDocumentStartEvent -> ok EventDocumentStart
        YamlDocumentEndEvent -> ok EventDocumentEnd
        YamlAliasEvent -> do
        ...
        YamlScalarEvent -> do
            ...
            ok $ EventScalar bs (bsToTag tagbs) style anchor
        YamlSequenceStartEvent -> do
            yanchor <- c_get_sequence_start_anchor er
            anchor <- if yanchor == nullPtr
                          then return Nothing
                          else fmap Just $ peekCString yanchor
            ok $ EventSequenceStart anchor
        YamlSequenceEndEvent -> ok $ EventSequenceEnd
        YamlMappingStartEvent -> do
            yanchor <- c_get_mapping_start_anchor er
            anchor <- if yanchor == nullPtr
                          then return Nothing
                          else fmap Just $ peekCString yanchor
            ok $ EventMappingStart anchor
        YamlMappingEndEvent -> ok $ EventMappingEnd
...
decode :: MonadIO m => B.ByteString -> Enumerator (Position, Event) m a
decode bs i = do
...
decodeFile :: MonadIO m => FilePath -> Enumerator (Position, Event) m a
...
parserParseOne' :: Parser
                -> IO (Either String (Maybe (Position, Event)))
